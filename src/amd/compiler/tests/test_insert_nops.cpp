...
create_bvh()
{
   ...
   instr->operands[3] = Operand(PhysReg(256 + 0), v1); /* node */
   instr->operands[4] = Operand(PhysReg(256 + 1), v1); /* tmax */
   instr->operands[5] = Operand(PhysReg(256 + 2), v3); /* origin */
   instr->operands[6] = Operand(PhysReg(256 + 5), v3); /* dir */
   instr->operands[7] = Operand(PhysReg(256 + 8), v3); /* inv dir */
...
BEGIN_TEST(insert_nops.lds_direct_vmem)
   for (amd_gfx_level gfx : {GFX11, GFX12}) {
      ...
      if (gfx >= GFX12) {
         ...
         //~gfx12! v4: %0:v[0-3] = image_bvh64_intersect_ray %0:s[0-3], s4: undef, v1: undef, %0:v[0], %0:v[1], %0:v[2-4], %0:v[5-7], %0:v[8-10] 1d unrm r128
